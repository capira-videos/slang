...
window.Slang.mathslang = (function() {
        ...
        function _compare(expectedValue, givenValue, _units) {
            ...
            if (expectedValue.fun) {
                switch (expectedValue.fun) {
                    case '#equals':
                        return match(expectedExp, givenValue, units);

                    case '#identic':
                        return matchSyntax(expectedExp, givenValue); // expectedUnits

                    case '#approx':
                        return matchApprox(expectedExp, givenValue, expectedValue.error, units);
                    case '#lt':
                        return parseFloat(givenValue) < parseFloat(expectedExp);
                    ...
                    case '#geq':
                        return parseFloat(givenValue) >= parseFloat(expectedExp);
                    case '#vecEquals':
                        return Slang._mathslang.vec.compare(expectedExp, givenValue);
                    default:
                        return expectedExp === givenValue;
                }
        ...
        function extractUnit(s) {
            var u = '';
            if ([/Km/, /km/, /dm/, /cm/, /mm/].some(function(e) {
                    return e.test(s);
                })) {
                u += 'distance ';
            }
            if ([/Kg/, /kg/, /g/, /mg/].some(function(e) {
                    return e.test(s);
                })) {
                u += 'weight ';
            }
            if ([/h/, /min/, /s/, /ms/].some(function(e) {
                    return e.test(s);
                })) {
                u += 'time ';
            }
            ...
            if (u.indexOf('distance') == -1 && q.length > 1 && q.some(function(e) {
                    var x = 'gs'.split('').every(function(c) {
                        return !e.length || c != e[0];
                    });
                    var y = 'Kkdc'.split('').every(function(c) {
                        return !e.length || c != e[e.length - 1];
                    });
                    return x && y;
                }))
                u = 'distance ' + u;;
        ...
        function _macro() {
            return Slang._mathslang.macro;
        }

        function _lex() {
            return Slang._mathslang.lexical;
        }

        function _syntax() {
            return Slang._mathslang.syntax;
        }

        function _semantix() {
            return Slang._mathslang.semantix;
        }
