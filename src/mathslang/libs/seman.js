...
window.Slang._mathslang_semantix = ( function() {
	//=============================================================================
	/**STRUCT**/
	//=============================================================================
	...
	function present(s) {
		...
		var syntax = Slang._mathslang_syntax.present(s);// Slang._mathslang_syntax.clean(syntax);
		return represent(syntax);
	}
//var represent_count = 0;
//var represent_space = '';
	// []Slang._mathslang_syntax.Q -> S
	...
	/*	represent_count += 1;
		...
		for(var i=1;i<represent_count;++i)
			represent_space += '\t';
		;*/
//console.log(represent_space+'represent(qs='+Slang._mathslang_syntax.string(qs)+')')
		var rs = new S();
...
//		represent_count -= 1;
	...
	// Slang._mathslang_syntax.Q -> Q
	function representQ(q) {
//console.log(represent_space+'representQ(q='+Slang._mathslang_syntax.stringQ(q)+')');
		var rq = new Q();
		rq.fact = q.fact.reduce(function(a,b){return a*b;}, 1);
		rq.imag = Slang._mathslang_imag.strParse(q.imag);
		q.exps.forEach(function(e) {
//console.log(represent_space+'represent(e.radix='+Slang._mathslang_syntax.stringQ(e.radix)+')');
			var s0 = represent(e.radix);
//console.log('e.radix');
//console.log(represent_space+'represent(e.power='+Slang._mathslang_syntax.stringQ(e.power)+')');
...
//console.log(represent_space+'representQ(q) -> '+rq.string())
	...
	// Q -> boolean
	function constQ(q) {
		return 0==Slang._mathslang_imag.len(q.imag)
			&& 0==q.sums.length;
	...
	function solidify(s) {
		...
		&& 0==s.queues[0].sums
		&& !isNaN(constPow)) {
			Slang._mathslang_imag.pow(s.queues[0].imag, constPow);
			s.expont = null;
		} else
		...
		return 0==s.offset
			...
			&& !s.expont
			&& 1==s.queues[0].fact
			&& 0==Slang._mathslang_imag.len(s.queues[0].imag)
	...
	function solidifyQ(q) {
		return 1==q.fact
			&& 0==Slang._mathslang_imag.len(q.imag)
			&& 1==q.sums.length
			&& 0==q.sums[0].offset
			&& !q.sums[0].expont
	...
	function identQ(q) {
		return q.fact * q.sums.reduce(function(a, s) {
			return a * s.calc();
		}, 1) == 1 && Slang._mathslang_imag.empty(q.imag);
	};
	// S | Q -> string
	function string(s) {
	...
	function stringQ(q) {
		return q.sums.reduce(function(a, s){
			return a + '(' + s.string() + ')';
		}, ''+q.fact+Slang._mathslang_imag.string(q.imag));
	};
	// S | Q -> S | Q
	function clone(s0) {
		var s1 = new S;
	...
	function cloneQ(q0) {
		var q1 = new Q;
		q1.fact = q0.fact;
		q1.imag = Slang._mathslang_imag.clone(q0.imag);
...
//console.log(represent_space+'expand(s='+s.string()+')');
		...
		while(i < s.queues.length) {
			...
			Slang._mathslang_imag.clear(s.queues[i].imag);
			s.queues[i].fact = 1;
...
//console.log(represent_space+'extract(q='+q.string()+')');
		...
		// sa := (0 + A0)
		var sa = new S(0, [new Q(q.fact, Slang._mathslang_imag.clone(q.imag))]);
...
//console.log(represent_space+'expansion(s0='+s0.string()+', s1='+s1.string()+')');
		...
		var qs = s0.queues.map(function(q) { // assert?
			return new Q(s1.offset * q.fact, Slang._mathslang_imag.clone(q.imag), q.sums);
		}).concat(s1.queues.map(function(q) { // assert?
			return new Q(s0.offset * q.fact, Slang._mathslang_imag.clone(q.imag), q.sums);
		}));
		// + (a0 + b0) * (a1 + b1)
		s0.queues.forEach(function(q0) {
			s1.queues.forEach(function(q1) {
				var imag = { };
				Slang._mathslang_imag.insert(imag, q0.imag);
				Slang._mathslang_imag.insert(imag, q1.imag);
				qs.push(new Q(q0.fact * q1.fact, Slang._mathslang_imag.sort(imag),
...
//console.log(represent_space+'combine(s='+s.string()+')');
		...
		s.queues.filter(
			...
			function(a, b) {
				return -Slang._mathslang_imag.sortFunc(a.imag, b.imag);
			}
		).forEach(
			// combine imaginary
			// k0*a^2 + k1*a + k2*a
			...
			// - QS == []
			function(q1) {
				if(  q0!=undefined	&& Slang._mathslang_imag.equals(q0.imag, q1.imag)
...
//console.log(represent_space+'powerfy(s='+s.string()+')');
		...
		&& s.expont) {
			...
			if(isNaN(v)) {
				...
				Slang._mathslang_imag.pow(q.imag, v);
			}
		}
//console.log(represent_space+'powerfy(s) -> '+s.string());
		return s;
	...
	return { present : present, represent : represent };
})
( );